...
class DashboardBridgeNode(Node):
    ...
    def vehicle_odom_cb(self, msg: VehicleOdometry) -> None:
        ...
        pose = self.ned_to_enu_pose(north=float(msg.position[0]), east=float(msg.position[1]), down=float(msg.position[2]), yaw=self.current_pose.yaw if self.current_pose else 0.0)
    ...
    def publish_offboard_control_mode(self) -> None:
        ...
        msg.position = self.mode in ("auto", "takeoff", "return_home", "hold", "idle", "survey_countdown", "orbit")
